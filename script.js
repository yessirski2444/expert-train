// 3D Hyper-Realistic Town with AI Avatars
...
class AIAvatar {
    ...
    generateText(){
        ...
        // Memory-influenced text
        if(this.aware && Math.random()<0.7){
            const memInfluence = this.memory.length>5 ? awareSentences.concat(this.memory.slice(-5).map(m=>`I remember ${m}.`)) : awareSentences;
...
for(let i=0;i<20;i++){
    ...
    const mesh = new THREE.Mesh(
        new THREE.CapsuleGeometry(0.5 + age/80, 1.5 + age/40, 4, 8),
        new THREE.MeshPhongMaterial({color: new THREE.Color(`hsl(${colorHue},70%,50%)`)})
    );
